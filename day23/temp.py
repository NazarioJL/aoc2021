...
STACKS = [[69, 57, 45, 31], [71, 59, 47, 33], [73, 61, 49, 35], [75, 63, 51, 37]]
...
def move_home(map):
    ...
    for stack_i, stack in enumerate(STACKS):
        ...
        for cell in HALL_CELLS:
            if all(
                (cell2 == cell and map[cell2] == stack_ch)
                or (cell2 != cell and map[cell2] == ".")
                for cell2 in range(
                    min(cell, hall_entrance), max(cell, hall_entrance) + 1
                )
            ):
                # Move home!
    ...
    for stack_i in range(4):
        ...
        for cell in HALL_CELLS:
            if all(
                ...
                new_map = (
                    new_map[0 : stack_tops[stack_i]]
                    + "."
                    + new_map[stack_tops[stack_i] + 1 :]
                )
                distance = (
                    distances_out[stack_i] + abs(cell - hall_entrance)
                ) * WEIGHTS[ch]
                # print(new_map)
                while True:
